...
financialForm.addEventListener("submit", function (event) {
  ...
  if (totalSavings < 0.1 * totalIncome) {
    ...
    savingsPlanAlertText.textContent = "";
  }
  ...
  document.querySelector("#savingsRatioText").textContent = savingsRatio.toFixed(2) + "%";
  ...
  // Виведення результатів
  document.querySelector("#investmentTypeText").textContent = investmentType;
});
